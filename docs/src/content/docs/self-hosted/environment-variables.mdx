...
| `TERRAT_WEB_BASE_URL` | Public-facing web base URL. The base of the run links in comments and commit checks for a repository whose brand has no base of its own | `https://terrateam.example.com` |
| `TERRAT_UI_BASE` | Public-facing UI base URL (required for UI). The base of the run links of a `terrateam`-brand repository | `https://terrateam.example.com` |
...
If a configuration change does not take effect, check
[`terrat_errors_total` and the other server metrics](/self-hosted/observability-metrics)
first. If that does not explain it, [contact support](/support) with your server
logs and your deployment version.
