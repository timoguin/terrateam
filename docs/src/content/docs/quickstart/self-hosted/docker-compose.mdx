...
The Terrateam setup wizard is a containerized application that handles all the complex configuration automatically for your chosen version control system. It is published as the `ghcr.io/stategraph/orchestration-setup` container image. The `setup` service in `docker/terrat/docker-compose.yml` pulls that image, and you can also run it on its own:

```sh
docker run --rm -p 3000:3000 ghcr.io/stategraph/orchestration-setup:latest
```

The previous image, `ghcr.io/terrateamio/terrateam-setup`, is no longer updated.
