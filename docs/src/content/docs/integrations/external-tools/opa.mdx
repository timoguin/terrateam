...
Terrateam also has an `opa` step type that runs `opa eval` against the JSON representation of the plan without Conftest. Pass the policy and the query with `extra_args`, and choose with `fail_on` whether a `defined` or an `undefined` query result fails the step (default `undefined`):
...
In this configuration the step fails when `data.main.violation` evaluates to a defined value, so `violation` must be a rule that is only defined when the plan violates the policy (a partial set rule such as `deny` is always defined, even when empty). See the [workflows reference](/reference/configuration/workflows#opa) for all keys.
