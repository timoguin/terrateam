...
Now let's add dependency management within each environment.  This creates one stack per layer, and `dev`, `staging`, and `production` stacks which nest the layers.  It also moves the `environment` variable to the parent stack, rather than defining it in each nested layer stack.  Finally, it also moves the rules which define when the `dev`, `staging`, and `production` stacks can be applied relative to each other into each stack's definition.
