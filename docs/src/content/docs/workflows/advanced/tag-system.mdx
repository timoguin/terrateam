...
| `in`          | `<glob> in dir` matches directories whose path contains a component matching `<glob>` (`**/<glob>/**`). Only `dir` is accepted on the right-hand side. | `app in dir`                   |
...
| `app in dir`                 | Matches directories that have a path component named `app`, such as `services/app/prod`. |
