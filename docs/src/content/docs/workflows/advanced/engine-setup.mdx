...
engine:
  ...
  tf_cmd: tofu
```
See the [Stategraph integration guide](/integrations/iac-tools/stategraph/) for the required secrets and `stategraph.json`.
...
engine:
  ...
  # The command to run for terrateam apply-autoapprove, which has no stored plan (optional)
...
**Highlights:**
- All steps (`init`, `plan`, `diff`, `resource_summary`, `apply`, `unsafe_apply`, `outputs`) are optional
...
* `resource_summary` The command to print the plan's resource counts as a JSON object with the integer keys `created`, `updated`, `replaced` and `deleted`. It runs after a plan that reports changes, and its counts appear in the status check description and the summary comment. Optional.
* `apply` The command to run during the apply step. Optional.
* `unsafe_apply` The command to run for [`terrateam apply-autoapprove`](/reference/commands/apply-autoapprove), which applies without a previously stored plan. Optional.
...
:::note
The exit code of the `plan` script is important.  An exit code of `0` means the plan succeeded and there are changes, and `2` means the plan succeeded but there are no changes.  Any other exit code is considered a failure.  Note that this is the reverse of the `-detailed-exitcode` convention of Terraform and OpenTofu, where `2` means changes; wrap those tools if you pass their exit code through.
:::
...
* Use `version` on the `terraform` and `tofu` engines, and `tf_cmd` and `tf_version` on the `terragrunt`, `cdktf`, and `stategraph` engines, to control which Terraform compatible CLI is invoked.
