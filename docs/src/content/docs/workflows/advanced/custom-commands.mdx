...
Besides the engine steps `init`, `plan`, and `apply`, Terrateam supports the `run`, `env`, `oidc`, `checkov`, `conftest`, `opa`, and `gates` step types in workflows. See the [workflows reference](/reference/configuration/workflows#workflow-steps) for the full list. The most common custom steps are:
...
4. After approval, comment `terrateam apply` (or merge, if `when_modified.autoapply` is `true`). Terrateam executes the custom apply workflow:
   ###### `terraform init` is executed
