...
```
In this example, changes to either the `network` or `database` directory will trigger the operation for `application`. Set `depends_on` under `dirs.<directory>.when_modified`; under the top-level `when_modified` it would apply to every directory, including the dependencies themselves.
