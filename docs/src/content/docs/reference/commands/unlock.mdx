...
terrateam unlock drift
```
Releases the locks held by [drift detection](/reference/configuration/drift) runs. `drift` can be combined with pull request numbers in one command.
...
- [terrateam apply](/reference/commands/apply) - Acquires locks on the applied directories and workspaces
- [lock_policy](/reference/configuration/lock-policy) - Controls when locks are acquired
