...
A stack is the unit that `depends_on` orders within. The [`depends_on`](/reference/configuration/when-modified/#depends-on-and-stacks) key of a directory may only name directories in the same stack, or in a stack that shares a nested stack with it. Naming a directory in an unrelated stack is a configuration error, and the run reports it instead of planning.
