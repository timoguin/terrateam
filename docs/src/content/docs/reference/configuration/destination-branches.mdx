...
- The `destination_branches` configuration is read from the repository's default branch as long as it is listed in [`default_branch_overrides`](/reference/configuration/default-branch-overrides), which it is by default.
