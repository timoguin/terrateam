...
A custom `run` step that shells out to `terraform plan` or `terraform apply` does not satisfy this requirement: the `plan` step is what stores the plan file, and the `apply` step is what consumes it.
