...
| schedules | Object | An object where the key is the unique name of the schedule and the value is the configuration for the schedule. Required in this form, even when `enabled` is `false` (use `schedules: {}`). |
...
| Key | Type | Description |
|-----|------|-------------|
| schedule | String | The interval to run drift detection and reconciliation: `hourly`, `daily`, `weekly`, `monthly`. Required. |
| reconcile | Boolean | Specifies whether reconciliation is enabled. Default is false. |
| tag_query | String | A tag query that specifies which directories and workspaces drift detection and reconciliation should be applied to. Required. Use `''` to match everything. |
| window | Object | The window that the scheduled drift can run in. Optional. When set, both `start` and `end` are required. |
...
:::note
If `end` is less than `start`, it is interpreted as crossing over to the next day.  For example if `start` is `21:00 EST` and `end` is `01:00 EST`, the window will start at `21:00 EST` today and end at `01:00 EST` tomorrow.
:::

### Legacy Single Schedule Form
A single schedule can also be written directly under `drift` with the `schedule`, `reconcile`, and `tag_query` keys. This form is equivalent to a `schedules` entry named `default`; `tag_query` defaults to matching everything and `window` is not available.

| Key | Type | Description |
|-----|------|-------------|
| enabled | Boolean | Specifies whether drift detection is enabled. Default is false. |
| schedule | String | The interval: `hourly`, `daily`, `weekly`, `monthly`. Required. |
| reconcile | Boolean | Specifies whether reconciliation is enabled. Default is false. |
| tag_query | String | A tag query limiting the directories and workspaces. Default matches everything. |
...
- `monthly`
:::info
There is no default value for `schedule`, and this key is required in every schedule.
:::
