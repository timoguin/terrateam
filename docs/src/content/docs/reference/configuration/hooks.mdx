...
| on_error | List | Actions to take if the command fails. Each entry is an object with a `type`. The only supported type is `gate`, which creates a [Gatekeeper](/governance/gatekeeper) gate with the keys `token`, `name`, `all_of`, `any_of`, and `any_of_count`. |

### `gates`
The `gates` hook type runs a command and creates a [Gatekeeper](/governance/gatekeeper) gate for each entry of the `gates` list in the JSON the command prints to standard output. The hook never fails the operation.
| Key | Type | Description |
|-----|------|-------------|
...
### `drift_create_issue`
The `drift_create_issue` hook type creates an issue in the repository when a [drift detection](/reference/configuration/drift) run finds drift. It is only meaningful as a `plan` post-hook of a drift run; it does nothing in other runs. It runs regardless of the success of the workflow.
| Key | Type | Description |
|-----|------|-------------|
