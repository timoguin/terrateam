...
| stacks | object | Stack configuration for the [CDKTF](/integrations/iac-tools/cdktf) engine. Same shape as `workspaces`: the key is the stack name and the value accepts `tags` and `when_modified`. |
