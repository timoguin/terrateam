...
```yaml
access_control:      # Specifies user permissions for operations
  policies: []
apply_requirements:  # Defines preconditions for applying changes
  checks: []
dirs: {}             # Maps directories to tags, workspaces, and behaviors
hooks:               # Executes custom commands before or after operations
  all:
    pre: []
    post: []
workflows: []        # Configures specialized plan and apply steps
```
  ...
  checks:
    - tag_query: ""  # Apply to all workspaces
      ...
      status_checks:
        ...
        ignore_matching:           # Patterns of check names to ignore
