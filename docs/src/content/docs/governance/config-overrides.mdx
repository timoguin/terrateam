...
By default, all other configuration settings (workflows, hooks, etc.) are read from the feature branch. Any top-level key can be added to `default_branch_overrides` to pin it to the default branch as well. The `default_branch_overrides` key itself is always read from the default branch, so a feature branch cannot remove a key from the list.
