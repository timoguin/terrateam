...
let fork_isolation =
  Oth.test ~desc:"forked chains see their own data" ~name:"Fork isolation" (fun _ ->
      ...
      Oth.Assert.true_ (!r_a = Some 11);
      Oth.Assert.true_ (!r_b = Some 22))
...
let concurrent_bind_isolation =
  Oth.test
    ...
    (fun _ ->
      ...
      Oth.Assert.true_ (List.rev !trace_a = [ `After_first 11; `After_second 12 ]);
      Oth.Assert.true_ (List.rev !trace_b = [ `After_first 21; `After_second 22 ]))
