...
module Assert = struct
  ...
  let eq_ ~eq ~(expected : 'a) ~(actual : 'a) =
    if not (eq expected actual) then
      raise (Assert_failure_msg "Expected things to be equal, but they aren't")
  ...
  module List = struct
    ...
    let str_mem ?fail_msg ~searched l =
      if not (CCList.mem ~eq:CCString.equal searched l) then
        false_
          (Format.asprintf
             "%sExpected the list to contain %S, got [%s]"
