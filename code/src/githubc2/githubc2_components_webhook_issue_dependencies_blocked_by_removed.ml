module Primary = struct
  module Action = struct
    let t_of_yojson = function
      | `String "blocked_by_removed" -> Ok `Blocked_by_removed
      | json -> Error ("Unknown value: " ^ Yojson.Safe.pretty_to_string json)

    let t_to_yojson = function
      | `Blocked_by_removed -> `String "blocked_by_removed"

    type t = ([ `Blocked_by_removed ][@of_yojson t_of_yojson] [@to_yojson t_to_yojson])
  ...
  type t = {
    ...
    blocking_issue_repo : Githubc2_components_repository.t option; [@default None]
