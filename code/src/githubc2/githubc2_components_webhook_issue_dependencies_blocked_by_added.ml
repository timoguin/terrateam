module Primary = struct
  module Action = struct
    let t_of_yojson = function
      | `String "blocked_by_added" -> Ok `Blocked_by_added
      | json -> Error ("Unknown value: " ^ Yojson.Safe.pretty_to_string json)

    let t_to_yojson = function
      | `Blocked_by_added -> `String "blocked_by_added"

    type t = ([ `Blocked_by_added ][@of_yojson t_of_yojson] [@to_yojson t_to_yojson])
