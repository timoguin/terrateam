...
let rec range from until =
  if Js.float_of_number from##getTime < Js.float_of_number until##getTime then
    from
    :: range
         (new%js Js.date_fromTimeValue
            (Js.number_of_float (Js.float_of_number from##getTime +. millisecond_day)))
         until
  else []

let add_milliseconds d ms =
  new%js Js.date_fromTimeValue (Js.number_of_float (Js.float_of_number d##getTime +. ms))

let now () = new%js Js.date_now
let get_time d = Js.float_of_number d##getTime
