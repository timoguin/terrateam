...
let fetch_branch_sha_cached ~request_id client repo ref_ = raise (Failure "nyi")
...
let create_commit_checks ~request_id ~brand client repo ref_ checks = raise (Failure "nyi")
